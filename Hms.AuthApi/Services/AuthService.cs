...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResponseDto> PatientLoginAsync(LoginRequestDto request)
    {
        ...
        var link = await _patientUserLinkRepository.GetByPatientIdAsync(request.PatientId)
            ?? throw new InvalidOperationException("Patient portal user was not created. Send OTP first.");
        ...
        if (!user.IsActive)
            throw new InvalidOperationException("This portal user is inactive.");

        if (!user.IsOtpLoginEnabled && user.IsFirstLoginCompleted)
            throw new InvalidOperationException("OTP login is disabled for this user.");
    ...
    public async Task SendStaffLoginOtpAsync(StaffOtpRequestDto request)
    {
        ...
        if (roles.Contains(AppRoles.Patient, StringComparer.OrdinalIgnoreCase))
            throw new InvalidOperationException("Use patient OTP login for patient accounts.");
    ...
    public async Task<AuthResponseDto> StaffLoginAsync(StaffLoginRequestDto request)
    {
        if (string.IsNullOrWhiteSpace(request.LoginId) || string.IsNullOrWhiteSpace(request.Password))
            throw new ArgumentException("Login id and password are required.");

        var user = await _userRepository.GetByLoginIdWithRolesAsync(request.LoginId.Trim())
            ?? throw new InvalidOperationException("Invalid login id or password.");

        if (!user.IsActive)
            throw new InvalidOperationException("This portal user is inactive.");

        if (!user.IsFirstLoginCompleted)
            throw new InvalidOperationException("First login must be completed with OTP. Then choose your authentication preference.");

        if (!user.IsPasswordLoginEnabled)
            throw new InvalidOperationException("Password/JWT login is not enabled for this user.");
    ...
    public async Task<AuthResponseDto> UpdateAuthPreferenceAsync(int userId, AuthPreferenceRequestDto request)
    {
        ...
        if (!request.EnablePasswordLogin && !request.EnableOtpLogin)
            throw new ArgumentException("Enable at least one login method.");
        ...
        if (request.EnablePasswordLogin)
        {
            if (string.IsNullOrWhiteSpace(request.LoginId))
                throw new ArgumentException("Login id is required when password login is enabled.");
            ...
            if (string.IsNullOrWhiteSpace(user.PasswordHash))
                throw new ArgumentException("Password is required the first time password login is enabled.");
        }
    ...
    public async Task<UserAdminResponseDto> CreateStaffUserAsync(CreateStaffUserRequestDto request)
    {
        ...
        if (!new[] { AppRoles.Admin, AppRoles.Doctor, AppRoles.Receptionist }
        ...
        if (existingMobile != null &&
            !existingMobile.UserRoles.All(r => r.Role.Name.Equals(AppRoles.Patient, StringComparison.OrdinalIgnoreCase)))
        {
        ...
        var user = new User
        {
            LoginId = string.IsNullOrWhiteSpace(request.LoginId)
                ? mobile
        ...
        bool isProfileCompleted)
    {
        ...
        {
            UserId = user.Id,
            PatientId = link?.PatientId ?? 0,
            UHID = link?.UHID ?? string.Empty,
            ...
            IsPasswordLoginEnabled = user.IsPasswordLoginEnabled,
            IsOtpLoginEnabled = user.IsOtpLoginEnabled,
    ...
    private async Task<(User User, string Mobile)> FindStaffUserForOtpAsync(string identifier)
    {
        ...
        if (user == null)
            throw new InvalidOperationException("No portal user found for this login id or mobile number.");

        if (string.IsNullOrWhiteSpace(mobile))
            throw new InvalidOperationException("This user does not have a registered mobile number for OTP login.");
    ...
    {
        var identifier = !string.IsNullOrWhiteSpace(loginId)
            ? loginId
            : mobileNumber;

        if (string.IsNullOrWhiteSpace(identifier))
            throw new ArgumentException("Login id or mobile number is required.");
